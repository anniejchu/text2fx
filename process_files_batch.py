...
python process_files_batch.py \
    ...
    3 \
    EQ reverb \
    --export_dir experiments/2024-07-09/checking_process_files_batch_4 \
    --learning_rate 0.01 \
...
def main(audio_dir: Union[str, Path],
    ...
    x = 1
    sampled_audio_files = tc.sample_audio_files(audio_dir, n)[x:x+1]
    in_sig_batch = tc.wavs_to_batch(sampled_audio_files)
    sampled_descriptions = tc.sample_words(descriptions_source, n)[x]
    ...
    if export_dir is not None:
        export_param_dict_path = Path(export_dir) / f'output_{x}.json'
